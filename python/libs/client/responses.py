...
class TapeRestStageResponse(Struct):
  """Response returned after submitting a Tape REST stage request.

  :var requestId: Server-side stage request identifier
  """
  def __init__(self, response):
    super(TapeRestStageResponse, self).__init__(response)

class TapeRestStageFileStatus(Struct):
  ...
  """
  def __init__(self, status):
    super(TapeRestStageFileStatus, self).__init__(status)

class TapeRestStageStatus(Struct):
  ...
  def __init__(self, status):
    status = dict(status)
    status['files'] = [TapeRestStageFileStatus(f)
                       for f in status.get('files', [])]
    super(TapeRestStageStatus, self).__init__(status)
