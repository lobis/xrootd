...
CurlTokenOp::Redirect(std::string &target)
{
    ...
    const auto authority_start = normalized.find("://") + 3;
    const auto authority_end = normalized.find_first_of("/?#", authority_start);
    if (normalized.substr(authority_start,
                          authority_end - authority_start).find('@') !=
        std::string::npos) {
        Fail(XrdCl::errErrorResponse, kXR_ServerError,
             "Refusing credentials in a token request redirect");
        return RedirectAction::Fail;
    }

    XrdCl::URL source(m_url);
    XrdCl::URL destination(normalized);
    if (!source.IsValid() || !destination.IsValid() ||
        source.GetHostName() != destination.GetHostName() ||
        source.GetPort() != destination.GetPort()) {
        Fail(XrdCl::errErrorResponse, kXR_ServerError,
             "Refusing to redirect a token request to another origin");
        return RedirectAction::Fail;
    }
...
CurlTokenOp::Success()
{
    if (GetStatusCode() != 200 &&
        !(m_response_key == "$poll" &&
          (GetStatusCode() == 400 || GetStatusCode() == 403 ||
           GetStatusCode() == 428))) {
        Fail(XrdCl::errErrorResponse, kXR_ServerError,
    ...
    std::string value;
    std::string error;
    if (m_response_key == "$json" || m_response_key == "$poll") {
        if (m_response.empty() || m_response.size() >= kMaxTokenResponseSize) {
            Fail(XrdCl::errInvalidResponse, kXR_ServerError,
                 "Invalid token endpoint JSON response");
            return;
        }
        value = m_response;
    } else {
