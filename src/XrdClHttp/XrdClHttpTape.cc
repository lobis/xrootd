...
{
  request.method = XrdClHttp::TapeHttpMethod::Post;
                           ...
                           Json &output, bool &present)
{
  ...
  if(!XrdOucJsonUtils::GetOptionalUnsignedInteger(
       input, key, value, present))
  {
    return false;
  }
  if(present) output[key] = value;
  return true;
}
  ...
  const Json &input, Json &output)
{
  ...
  {
    output["path"] = NormalizeTapePath(input["path"].get<std::string>());
  }
  else
  {
    return ErrorStatus(XrdCl::errInvalidResponse,
  ...
  if(input.contains("error") && !input["error"].is_string())
  {
    return ErrorStatus(XrdCl::errInvalidResponse,
      "stage request response contains a non-string error field");
  }
  if(input.contains("error")
     && !input["error"].get_ref<const std::string &>().empty())
  {
                       ...
                       const std::string &path)
{
  ...
  const std::string canonical = CanonicalLocality(locality);
  if(canonical.empty())
  {
    result["error"] = "file locality reported as \"" + locality + "\"";
  }
  else result["locality"] = canonical;
  return result;
}
