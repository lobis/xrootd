                             ...
                             std::string &error)
{
    ...
    if (type == parsed.end() || !type->is_string()) {
        error = "Token query requires a string 'type'";
        return false;
    }
    const auto type_name = type->get<std::string>();
    if (type_name == "macaroon") request.type = TokenRequest::Type::Macaroon;
    else if (type_name == "oauth") request.type = TokenRequest::Type::OAuth;
    else if (type_name == "oauth-macaroon")
        request.type = TokenRequest::Type::OAuthMacaroon;
    else if (type_name == "device-start")
        request.type = TokenRequest::Type::DeviceStart;
    else if (type_name == "device-poll")
        request.type = TokenRequest::Type::DevicePoll;
    else {
        error = "Unknown token query type";
        return false;
    }
    ...
    request.path = path == parsed.end() ? "/" : path->get<std::string>();
    ...
    if (request.type == TokenRequest::Type::Macaroon && !request.issuer.empty()) {
        error = "Direct macaroon requests must not include an issuer";
        return false;
    }
    if (request.type != TokenRequest::Type::Macaroon && request.issuer.empty()) {
        error = "OAuth token requests require an issuer";
        return false;
    }

    for (const auto *name : {"scope", "client_id", "client_secret",
                             "device_code", "device_endpoint",
                             "token_endpoint"}) {
        auto member = parsed.find(name);
        if (member != parsed.end() && !member->is_string()) {
            error = std::string("Token query '") + name + "' must be a string";
            return false;
        }
    }
    request.scope = parsed.value("scope", std::string{});
    request.client_id = parsed.value("client_id", std::string{});
    request.client_secret = parsed.value("client_secret", std::string{});
    request.device_code = parsed.value("device_code", std::string{});
    request.device_endpoint = parsed.value("device_endpoint", std::string{});
    request.token_endpoint = parsed.value("token_endpoint", std::string{});
    if ((request.type == TokenRequest::Type::OAuth ||
         request.type == TokenRequest::Type::DeviceStart) &&
        request.scope.empty()) {
        error = "OAuth access token requests require a scope";
        return false;
    }
    if (request.type == TokenRequest::Type::OAuth &&
        request.client_id.empty()) {
        error = "OAuth client credentials require a client ID";
        return false;
    }
    if ((request.type == TokenRequest::Type::DeviceStart ||
         request.type == TokenRequest::Type::DevicePoll) &&
        request.client_id.empty()) {
        error = "Device authorization requires a client ID";
        return false;
    }
    if (request.type == TokenRequest::Type::DevicePoll &&
        (request.device_code.empty() || request.token_endpoint.empty())) {
        error = "Device polling requires a code and token endpoint";
        return false;
    }
    if (request.type == TokenRequest::Type::DeviceStart &&
        request.device_endpoint.empty() != request.token_endpoint.empty()) {
        error = "Device and token endpoints must be supplied together";
        return false;
    }
...
std::string
XrdClHttp::BuildOAuthRequest(std::string_view scope,
                             std::string_view client_id,
                             std::string_view client_secret)
{
    std::string body = "grant_type=client_credentials&scope=" +
                       PercentEncode(scope);
    if (!client_id.empty()) body += "&client_id=" + PercentEncode(client_id);
    if (!client_secret.empty())
        body += "&client_secret=" + PercentEncode(client_secret);
    return body;
}

std::string
XrdClHttp::BuildDeviceAuthorizationRequest(std::string_view scope,
                                            std::string_view client_id)
{
    return "client_id=" + PercentEncode(client_id) +
           "&scope=" + PercentEncode(scope);
}

std::string
XrdClHttp::BuildDeviceTokenRequest(std::string_view device_code,
                                    std::string_view client_id,
                                    std::string_view client_secret)
{
    std::string body =
           "grant_type=urn%3Aietf%3Aparams%3Aoauth%3Agrant-type%3Adevice_code"
           "&device_code=" + PercentEncode(device_code) +
    ...
    std::string &error)
{
    ...
    body = BuildOAuthRequest(scopes, client_id, client_secret) +
        "&expire_in=" + std::to_string(validity * 60);
    return true;
}
