...
class TokenIssuerHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith(metadata_prefix):
            ...
            if scenario == "slow-deadline":
                ...
                return
            if scenario in ("oauth-success", "oauth-failure",
                            "device-success"):
                metadata = {"token_endpoint": self.token_endpoint(scenario)}
                if scenario == "device-success":
                    metadata["device_authorization_endpoint"] = (
                        "{}/device/{}".format(
                            self.server.advertised_url, scenario
                        )
                    )
                self.send_json(
                    200, metadata
        ...
        if self.path == "/slow-deadline/.well-known/openid-configuration":
            time.sleep(3)
    ...
    def do_POST(self):
        ...
        if self.path == "/device/device-success":
            if "client_id=id" in body and "scope=" in body:
                self.send_json(200, {
        ...
        if self.path == "/device/device-google":
            self.send_json(200, {
                "device_code": "google-device-code",
                "user_code": "GOOGLE-CODE",
                "verification_url": "https://example.test/google-verify",
                "expires_in": 30,
                "interval": 1,
            })
            return

        if self.path == "/token/device-google":
            with self.server.trace_lock:
                self.server.device_google_polls += 1
                count = self.server.device_google_polls
            if "device_code=google-device-code" not in body:
                self.send_json(400, {"error": "invalid_grant"})
            elif count == 1:
                self.send_json(428, {"error": "authorization_pending"})
            else:
                self.send_json(200, {"id_token": "google-device-id-token"})
            return

        if self.path == "/token/device-success":
            with self.server.trace_lock:
                self.server.device_polls += 1
                count = self.server.device_polls
            if "device_code=device-code" not in body:
                self.send_json(400, {"error": "invalid_grant"})
            elif count == 1:
                self.send_json(400, {"error": "authorization_pending"})
            else:
                self.send_json(200, {
        ...
        if self.path == "/token/oauth-success":
            if "&scope=" in body and "&client_secret=" in body:
